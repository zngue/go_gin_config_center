...
import (
	...
	req "github.com/zngue/go_gin_config_center/app/request/jwt"
)

func (s *Service) List ( request req.ListRequest ) (num int,mb []*jwt.Jwt,err error ) {
	dbConn:=s.Jwt().GetList()
	...
	b,err:=request.OnlyCount(func() error {
		return dbConn.Model(&jwt.Jwt{}).Count(&num).Error
	})
	if b || err!=nil{
		return
	}
